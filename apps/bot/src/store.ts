...
export interface Warning { id: number; userId: string; moderatorId: string; reason: string; createdAt: string; }
...
async function load(): Promise<Record<string, GuildData>> {
  ...
  try { cache = JSON.parse(await readFile(filePath, 'utf8')) as Record<string, GuildData>; }
  catch { cache = {}; }
  return cache;
}

async function syncFromSupabase(guildId: string, value: GuildData): Promise<GuildData> {
  if (!process.env.SUPABASE_URL || (!process.env.SUPABASE_SERVICE_ROLE_KEY && !process.env.SUPABASE_ANON_KEY)) return value;
  const remote = await loadGuildRecord(guildId);
  if (!remote) return value;
  return { ...value, ...remote, config: { ...value.config, ...(remote.config as Partial<GuildData['config']> | undefined) } } as GuildData;
}
...
export async function save(): Promise<void> {
  ...
  writeQueue = writeQueue.then(async () => {
    ...
    if (process.env.SUPABASE_URL && (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY)) for (const [guildId, guild] of Object.entries(data)) await saveGuildRecord(guildId, guild as unknown as Record<string, unknown>);
  });
  await writeQueue;
}

export async function saveGuild(guildId: string, guild: GuildData): Promise<void> {
  await save();
  if (process.env.SUPABASE_URL && (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY)) await saveGuildRecord(guildId, guild as unknown as Record<string, unknown>);
}

export function statsFor(guild: GuildData, userId: string): MemberStats {
  guild.stats[userId] ??= { xp: 0, level: 0, messages: 0, invites: 0, balance: 0 };
  return guild.stats[userId];
}

export function addAudit(guild: GuildData, action: string, actorId?: string, targetId?: string, metadata?: unknown): void {
  guild.audit.unshift({ action, actorId, targetId, metadata, createdAt: new Date().toISOString() });
