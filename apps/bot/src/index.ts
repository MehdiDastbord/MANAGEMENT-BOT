...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    else if (interaction.commandName === 'pollvote') await pollvote(interaction, guild);
    else if (interaction.commandName === 'announce') await announce(interaction);
...
async function rep(interaction: import('discord.js').ChatInputCommandInteraction, guild: Awaited<ReturnType<typeof guildData>>) { const user = interaction.options.getUser('user', true); if (user.id === interaction.user.id) return interaction.reply({ content: 'You cannot give reputation to yourself.', ephemeral: true }); const amount = interaction.options.getInteger('amount', true); const stats = statsFor(guild, user.id); stats.reputation = (stats.reputation ?? 0) + amount; addAudit(guild, 'reputation.give', interaction.user.id, user.id, { amount, reason: interaction.options.getString('reason') }); await interaction.reply(`${user} received ${amount} reputation.`); }
async function poll(interaction: import('discord.js').ChatInputCommandInteraction, guild: Awaited<ReturnType<typeof guildData>>) { const options = interaction.options.getString('options', true).split(',').map(value => value.trim()).filter(Boolean).slice(0, 5); if (options.length < 2) return interaction.reply({ content: 'Provide at least two comma-separated options.', ephemeral: true }); const id = guild.polls.length + 1; guild.polls.push({ id, question: interaction.options.getString('question', true), options, votes: {}, closed: false, createdAt: new Date().toISOString() }); await interaction.reply(`Poll #${id}\n**${guild.polls[id - 1].question}**\n${options.map((option, index) => `${index + 1}. ${option}`).join('\n')}\n\nVote with: /pollvote poll:${id} option:1`); }
async function pollvote(interaction: import('discord.js').ChatInputCommandInteraction, guild: Awaited<ReturnType<typeof guildData>>) { const pollRecord = guild.polls.find(item => item.id === interaction.options.getInteger('poll', true)); const option = interaction.options.getInteger('option', true); if (!pollRecord || pollRecord.closed || !pollRecord.options[option - 1]) return interaction.reply({ content: 'That poll or option does not exist.', ephemeral: true }); pollRecord.votes[interaction.user.id] = option; const counts = pollRecord.options.map((label, index) => `${index + 1}. ${label}: ${Object.values(pollRecord.votes).filter(value => value === index + 1).length}`).join('\n'); await interaction.reply({ content: `Vote recorded.\n${counts}`, ephemeral: true }); }
async function announce(interaction: import('discord.js').ChatInputCommandInteraction) { if (!interaction.channel?.isTextBased() || !('send' in interaction.channel)) return interaction.reply({ content: 'This requires a text channel.', ephemeral: true }); await interaction.channel.send({ embeds: [new EmbedBuilder().setTitle('Announcement').setDescription(interaction.options.getString('message', true)).setColor(0x2864d7)] }); await interaction.reply({ content: 'Announcement sent.', ephemeral: true }); }
